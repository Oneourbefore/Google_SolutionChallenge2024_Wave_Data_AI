...
news_df = pd.DataFrame({'date': news_dates, 'title': news_titles, 'link': final_urls, 'content': news_contents, 'image-url': news_image_url})

news_df = news_df.drop_duplicates(keep='first', ignore_index=True)
print("중복 제거 후 행 개수:", len(news_df))

# '해당 기사는 사진이 없습니다.'를 포함한 행 제거
news_df = news_df[news_df['image-url'] != '해당 기사는 사진이 없습니다.']
...
news_df.to_json('{}_{}.json'.format(search, now.strftime('%Y%m%d_%H시%M분%S초')), orient='records', force_ascii=False)
